# 图 5-3: 双流 vs 单流架构 POD 系数轨迹对比 — 同一外推算例，仅展示快群 4 阶
import matplotlib
matplotlib.use('Agg')
...
ABL  = '2D-PINN/ablation/no_amp_shape/results'
...
case_idx = 0  # representative extrapolation case
...
for m in range(n_modes):
    ...
    # Row 1: ablation (single-stream)
    ax1 = axes[1, m]
    ax1.plot(t, A_true[case_idx, :, m], color='#1565C0', linewidth=1.2, label='真实值')
    ax1.plot(t, A_abl[case_idx, :, m], color='#E53935', linewidth=1.0,
             linestyle='--', dashes=(4, 2), label='单流(no_amp)')
    ax1.grid(True, linestyle=':', alpha=0.4)
    ax1.tick_params(labelsize=14)
    if m == 0:
        ax1.set_ylabel('单流架构\n(no_amp_shape)', fontsize=20, fontweight='bold')
        ax1.legend(fontsize=14, loc='upper left')

plt.tight_layout(pad=0.8, h_pad=1.5, w_pad=0.6)
out = 'fig/fig_ch5_dual_vs_single.svg'

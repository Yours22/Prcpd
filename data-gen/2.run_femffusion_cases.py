...
def run_batch():
    # 检查执行文件是否存在 (如果是 WSL 跑 .exe，确保路径正确无误)
    if not os.path.exists(EXECUTABLE):
        print(f"警告: 找不到可执行文件 {EXECUTABLE}，请确认路径。")
        # 根据你的实际情况，如果是全局环境变量调用的程序，可以注释掉上面两行

    # 获取所有生成的 prm 文件，并按编号排序保证执行顺序
    prm_files = sorted(glob.glob(os.path.join(PRM_DIR, "*.prm")))
    ...
    with open(TIMING_CSV, 'w', newline='') as csvfile:
        ...
        for i, prm_path in enumerate(prm_files, 1):
            ...
            try:
                # 执行指令，并将标准输出屏蔽，避免终端被满屏的求解器迭代信息淹没
                # 如果你想看实时日志，可以把 stdout=subprocess.DEVNULL 删掉
                ...
                # 捕获 FEMFFUSION 运行崩溃 (例如矩阵发散、物理参数不合理)
                end_time = time.time()
                elapsed_time = end_time - start_time
                ...
                # 写入 CSV (状态记为 Failed，防止后续数据处理时读入坏数据)
                csv_writer.writerow([case_id, f"{elapsed_time:.3f}", "Failed"])
                
            # 强制将内存中的数据刷入硬盘，防止脚本中途被强杀导致记录丢失
            csvfile.flush() 

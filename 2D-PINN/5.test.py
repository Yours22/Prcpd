...
def evaluate_test_set(model, ckpt, data_prefix, out_subdir):
    ablation_mode = ABLATION_MODE
    no_symlog = (ablation_mode == 'no_symlog')
    no_amp = (ablation_mode == 'no_amp_shape')

    output_name = 'val' if data_prefix == 'val' else 'test_extrap'
    set_label = 'Val (in-distribution)' if data_prefix == 'val' else 'Test (extrapolation)'

    out_dir = os.path.join(RESULT_DIR, output_name)
    ...
    log_print(log_file, f"Ablation: {ablation_mode}")
    log_print(log_file, f"数据集: {set_label}")
    log_print(log_file, f"模型: {ROOT}/models/best_pod_lstm.pth\n")
    ...
    if no_amp:
        # 直接输出 8 个模态
        if no_symlog:
            A_pred = pred_out_raw * A_std + A_mean
        else:
            A_pred_symlog = pred_out_raw * A_std + A_mean
            A_pred = np.sign(A_pred_symlog) * np.expm1(np.abs(A_pred_symlog))
    else:
        pred_m1_scaled = pred_out_raw[:, :, 0:1]
        pred_R = pred_out_raw[:, :, 1:]
        if no_symlog:
            pred_m1_phys = pred_m1_scaled * A_std[:, :, 0:1] + A_mean[:, :, 0:1]
        else:
    ...
    rel_error_per_sample = np.linalg.norm(Y_pred - Y_raw, axis=(1, 2)) / (np.linalg.norm(Y_raw, axis=(1, 2)) + 1e-10)
...
def main():
    ckpt_path = os.path.join(ROOT, 'models', 'best_pod_lstm.pth')
    ckpt = torch.load(ckpt_path, map_location=device)

    no_amp = (ABLATION_MODE == 'no_amp_shape')
    output_dim = POD['r_fast'] + POD['r_thermal']
    # input_dim from checkpoint X_mean shape
    ...
    for data_prefix in ['val', 'test']:
        ...
        print(f"{'='*60}")
        evaluate_test_set(model, ckpt, data_prefix,
                          'val' if data_prefix == 'val' else 'test_extrap')

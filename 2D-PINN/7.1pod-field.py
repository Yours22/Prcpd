...
def main():
    ...
    ax.set_title('平均场', fontsize=10.5, fontweight='bold')
    ax.axis('off')
    ...
    for i in range(3):
        ...
        ax.set_title(f'空间模态 {i + 1}（{ratio:.4f}%）', fontsize=10.5, fontweight='bold')
        ax.axis('off')
    ...
    ax.text(0.5, 0.5, f'快群时间演化\n（算例 {TARGET_CASE}）',
            fontsize=14, fontweight='bold', ha='center', va='center', color='#37474F')
    ...
    for i in range(3):
        ...
        ax.set_title(f'模态 {i + 1} 时间系数', fontsize=10.5, fontweight='bold')
        ax.set_xlabel('时间步', fontsize=9)
        ax.set_ylabel(f'$a_{{{i + 1}}}(t)$', fontsize=9)
    ...
    save_path = os.path.join(PATHS['output_dir'], "pod_spatiotemporal_modes.svg")
